...
def RayTracing3DTOF(
    ...
    Parameters:
        GridSize: dimensions of the voxel grid [Nx Ny Nz]
        GridBounds: 1-by-6 matrix with voxel grid boundaries [xMin yMin zMin xMax yMax zMax] (must satisfy xMax>xMin, yMax>yMin, zMax>zMin)
        LineCoordinates: 1-by-6 matrix with line segment coordinates [x1,y1,z1,x2,y2,z2]
    
    Returns:      
        IntersectedVoxelData: N-by-1 matrix containing lenghts inside the intersected voxels
        IntersectedVoxelIndices: N-by-1 matrix containing linear indeces of the intersected voxels
        IntersectedVoxelSamples: N-by-1 matrix containing the sample point at the middle of the intersected voxels
    """
...
@jit(nopython=True)
def sub2ind(X, Y, Z, GridSize):
    return X + (Y - 1) * GridSize[1] + (Z - 1) * GridSize[1] * GridSize[0]

@jit(nopython=True)
def BoxIntersectTest(grid_size, grid_bounds, line_coordinates):
