...
%__________________________________________________________________________________________________________________
function Scatters = SingleScatterSimulation(ActivityMap, AttenuationMap, ImageSize,...
    ...
    Scatters = zeros(NrDetectors+1,NrDetectors/2,NrSinograms, 'single');
    ...
    parfor zSampleIndex = 1:numel(zSamplePoints)          % parellel for-loop to make algorithm faster
        ...
        ScatterSlice = zeros(NrDetectors+1,NrDetectors/2,NrSinograms, 'single');
        
        % structure to save how many times a LOR is used
        ScatterCounts = zeros(NrDetectors+1,NrDetectors/2,NrSinograms, 'single');
                    ...
                    if AttenuationTissue(xIndex,yIndex,zIndex) > 0
                        ...
                        for RingIndex1 = 1:NrRingsUsed
                            ...
                            for DetectorIndex1 = 1:NrDetectorsUsed
                                ...
                                [Lenghts, Indexes] = RayTracing3D(GridSize,GridBounds,LineCoordinates);
                                ActivityIntegral = sum(ActivityMap(Indexes).*Lenghts);
                                    ...
                                    if abs(RingIndex2 - RingIndex1) <= NrRingsUsed
                                        ...
                                        for DetectorIndex2 = 1:NrDetectorsUsed
                                            ...
                                            if (Detector1 == Detector2 && RingIndex2 == RingIndex1) || Detector1 > Detector2
                                                ...
                                                Probability = GeometricalEfficiency*EnnergyEfficiency*SetupEfficiency*(AttenuationPath1*AttenuationScaled2*ActivityPath1 + AttenuationPath2*ActivityPath2*AttenuationScaled1);
...
function InterpolatedSinograms = InterpolateAllBins(Sinograms, Rings, SinogramIndex, Detectors, SinogramCoordinates, SavePath)
    ...
    for i=1:size(Sinograms,3)
        ...
        Sinograms(:,:,i) = F({1:RadialDim,1:AngularDim});
    end
    ...
    [RadialDim,AngularDim,~] = size(Sinograms);
    Sinograms = reshape(Sinograms, RadialDim, AngularDim, NrRingsUsed, NrRingsUsed);
    Sinograms = permute(Sinograms, [1, 2, 4, 3]);
    % some grid vectors had their orientation changed to warn interpn that they correspond indeed to grid vectors
    SinogramsInterpolated = single(interpn(1:RadialDim, 1:AngularDim, Rings(:), Rings(:), Sinograms, 1:RadialDim, 1:AngularDim, (1:NrRings)', (1:NrRings)', 'linear'));
    clear Sinograms
    SinogramsInterpolated = reshape(SinogramsInterpolated, RadialDim, AngularDim, NrRings^2);
    ...
    filename = sprintf('%s/SSS.bin', SavePath);
    fileID = fopen(filename,'W');
    fwrite(fileID,SinogramsInterpolated(:,:,SinogramOrder),'float32');
    fclose(fileID);

    InterpolatedSinograms = SinogramsInterpolated(:,:,SinogramOrder);
    clear SinogramsInterpolated
end
%%_______________________________________________________________________________________________________________________________________________________________________________________________
