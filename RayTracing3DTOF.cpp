...
double *IntersectedVoxelData, *IntersectedVoxelSamples;
...
plhs[0] = mxCreateNumericMatrix(StorageSize,1,mxDOUBLE_CLASS,mxREAL); // initializes to zeros
IntersectedVoxelData = (double*)mxGetData(plhs[0]);
...
plhs[2] = mxCreateNumericMatrix(StorageSize,1,mxDOUBLE_CLASS,mxREAL); // initializes to zeros
IntersectedVoxelSamples = (double*)mxGetData(plhs[2]);
...
while ((X!=xEnd)||(Y!=yEnd)||(Z!=zEnd))
{
    ...
    if (AddedVoxelCount>StorageSize)
    {
        ...
        IntersectedVoxelData = (double*)mxRealloc(IntersectedVoxelData, sizeof(double)*StorageSize);
        IntersectedVoxelSamples = (double*)mxRealloc(IntersectedVoxelData, sizeof(double)*StorageSize);
        IntersectedVoxelIndices = (int*)mxRealloc(IntersectedVoxelIndices, sizeof(int)*StorageSize);
    }
    IntersectedVoxelData[AddedVoxelCount-1] = sqrt(pow(xGridStart - xVec*length,2) + pow(yGridStart - yVec*length,2) + pow(zGridStart - zVec*length,2));
...
IntersectedVoxelData = (double*)mxRealloc(IntersectedVoxelData, sizeof(double)*AddedVoxelCount);
IntersectedVoxelSamples = (double*)mxRealloc(IntersectedVoxelSamples, sizeof(double)*AddedVoxelCount);
...
mxSetM(plhs[2], AddedVoxelCount); //number of rows
mxSetN(plhs[2], 1); //number of columns
mxSetData(plhs[2], IntersectedVoxelSamples); // update pointer to the matrix data
}
